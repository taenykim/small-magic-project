  ...
  componentDidMount() {
    window.addEventListener(
      ...
      _.debounce(() => {
        ...
        if (window.scrollY + document.documentElement.clientHeight > document.documentElement.scrollHeight - 260 && this.state.imageMaxNumber >= this.state.imageNumber) {
          this.setState(
            {
    ...
    fetch(url)
      ...
      .then((text) => {
        ...
        let json = [],
          ...
          link,
          img;
        const maxNum = Number($("#page_container > h1").text().split(" ")[8]);
  ...
  render() {
    return (
      <BackgroundContainer>
        ...
        <CrawlingContainer>
          <PageView>{this.state.imageNumber <= this.state.imageMaxNumber ? <>{this.state.imageNumber + "/" + this.state.imageMaxNumber}</> : <>Last</>}</PageView>
          <SearchingContainer>
          ...
          <DescriptionContainer>
            <div>
              데스크탑 이용 시, <span style={{ color: "blue", fontFamily: "escore7" }}>width가 1070 이상</span>이어야 원활한 사용이 가능합니다.
            </div>
            <div>
